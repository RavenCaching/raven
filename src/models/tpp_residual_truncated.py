...
class RecurrentTPPResidualTruncated(nn.Module):
    """
    ...
    def __init__(
        ...
        self.mlp_tao_input = self.context_size
        if self.consider_objid:
            # input is [RNN hidden state, object ID]
            self.mlp_tao_input = self.context_size + 1

        self.mlp_tao = nn.Sequential(
            nn.Linear(self.mlp_tao_input, 3 * self.num_mix_components),
            nn.ReLU(),
            nn.Linear(3 * self.num_mix_components, 3 * self.num_mix_components),
            nn.ReLU(),
            nn.Linear(3 * self.num_mix_components, 3 * self.num_mix_components)
        )
        # to infer residual distribution, i.e., truncated distribution
        # input is tao distribution parameters, age
        self.mlp_r = nn.Sequential(
            nn.Linear(3 * self.num_mix_components + 1, 3 * self.num_mix_components),
            nn.ReLU(),
            nn.Linear(3 * self.num_mix_components, 3 * self.num_mix_components),
            nn.ReLU(),
            nn.Linear(3 * self.num_mix_components, 3 * self.num_mix_components),
    ...
    def get_inter_time_dist(self, context: torch.Tensor) -> torch.distributions.Distribution:
        raw_params = self.mlp_tao(context)  # (batch_size, seq_len, 3 * num_mix_components)
    ...
    def log_prob_tao(self, batch: Batch) -> torch.Tensor:
        ...
        inter_time_dist = self.get_inter_time_dist(context)  # (batch_size, seq_len)
    ...
    def get_residual_time_dist(self, context: torch.Tensor) -> torch.distributions.Distribution:
        raw_params = self.mlp_r(context)  # (batch_size, seq_len, 3 * num_mix_components)
        # Slice the tensor to get the parameters of the mixture
        locs = raw_params[..., :self.num_mix_components]
        log_scales = raw_params[..., self.num_mix_components: (2 * self.num_mix_components)]
        log_weights = raw_params[..., (2 * self.num_mix_components):]
    ...
    def log_prob_residual(self, batch: Batch) -> torch.Tensor:
        """Compute log-likelihood for a batch of sequences.
        Args:
            batch:
        Returns:
        ...
        if self.consider_objid:
            ...
            obj_ids = (obj_ids - self.mean_log_inter_time) / self.std_log_inter_time
            context = torch.cat([context, obj_ids], dim=-1)
        inter_time_raw_params = self.mlp_tao(context) # (batch_size, seq_len, 3 * num_mix_components)

        ages, residual_times = self.get_age_features(batch)  # (batch_size, seq_len, 1)
        context_residual = torch.cat([inter_time_raw_params, ages], dim=-1)
        residual_time_dist = self.get_residual_time_dist(context_residual)  # (batch_size, seq_len)
