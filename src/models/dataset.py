...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def train_val_split_within_seq(
            self, train_size=0.8, val_size=0.2, seed=None, shuffle=True,
    ) -> Tuple["SequenceDataset", "SequenceDataset"]:
        """Split the sequences into train, validation and test subsets."""
        if train_size < 0 or val_size < 0:
            raise ValueError("train_size, val_size must be >= 0.")
        if train_size + val_size != 1.0:
            raise ValueError("train_size, val_size must add up to 1.")

        if seed is not None:
            np.random.seed(seed)

        all_idx = np.arange(len(self))  # number of total sequences
        if shuffle:
            np.random.shuffle(all_idx)

        train_sequences = []
        val_sequences = []
        # split each sequences into train, valid, test parts
        for idx in all_idx:
            train_end = int(train_size * len(self.sequences[idx])) # idx of the last train sample within the sequence
            train_sequences.append(Sequence(
                    inter_times=self.sequences[idx].inter_times[:train_end],
                    marks = None,
            ))
    ...
    def get_latest_observations(self, last_step=64) -> "SequenceDataset":
        latest_sequences = []
        for idx in np.arange(len(self)):
            latest_sequences.append(Sequence(inter_times=self.sequences[idx].inter_times[-last_step:],
                                             marks=None))
        return SequenceDataset(latest_sequences, num_marks=self.num_marks)
