...
from .utils import DotDict, pad_sequence
...
class Batch(DotDict):
    ...
    def from_list(sequences: List[Sequence]):
        ...
        if sequences[0].marks is not None:
            ...
            marks = None
        #obj_id = cache.warm_index if hasattr(cache, 'warm_index') else -1
        if hasattr(sequences[0], 'obj_id'):
            obj_ids = pad_sequence([torch.Tensor([seq.obj_id] * len(seq.inter_times)) for seq in sequences], max_len=max_seq_len)
        else:
            obj_ids = None
        if hasattr(sequences[0], 'obj_size'):
            obj_sizes = pad_sequence([torch.Tensor([seq.obj_size] * len(seq.inter_times)) for seq in sequences], max_len=max_seq_len)
        else:
            obj_sizes = None
        return Batch(inter_times, mask, marks, obj_ids=obj_ids, obj_sizes=obj_sizes)
